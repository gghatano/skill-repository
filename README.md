...
| プラグイン | 用途 | 導入コマンド |
| --- | --- | --- |
| `deep-research` | 調査ワークフロー（依頼固定 → 収集 → 引用検証 → 出荷判定） | `/plugin install deep-research@gghatano-skills` |
...
この取りまとめの**手順と考え方（Input / Process / Output と検証）**を、別環境でも再現できる形で
整理した方法論は `docs/aggregation-methodology.md` にまとめています。
...
- `docs/design/`: 個別 Skill Pack の設計書（例 `deep-research-skill-pack.md`）
...
- `docs/plugins/<name>.html`: 各プラグインの**詳細ページ**（含まれるスキル・同梱物・導入後の調整・導入手順）。Pages のプラグインカードからリンク
