...
> ブランチ・worktree の命名、既定ブランチ、参照ドキュメントは規約
> `.claude/docs/dev-workflow-conventions.md`（§2 ブランチ・worktree 方針。既定 `develop` /
> `issue/<n>-<slug>` / `../worktrees/<repo>-issue-<n>`）に従う。別プロジェクトでは既定値を
> 読み替える。以下のコマンド（`uv sync` 等）はプロジェクトの環境構築手順に置き換える。

1 worktree = 1 Issue = 1 branch。複数 Issue を同一 worktree で扱わない。
詳細・コマンド例は**ブランチ方針**（既定 `docs/branch-worktree-policy.md`）。
