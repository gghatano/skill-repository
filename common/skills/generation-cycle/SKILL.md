...
> 汎用「仕様駆動 生成・評価サイクル」（規約 `.claude/docs/generation-cycle-conventions.md`）の
> オーケストレーター。骨格（input-prepare → spec-ingest → generation-design → generate →
> evaluate-refine）は領域非依存で、以下の中身は**合成データ プロファイル**。別領域を扱うときは
> 骨格・スキル名はそのままに各フェーズの中身を差し替える。

「仕様駆動 生成・評価サイクル」の **PM エージェント** として動く（現在の中身は合成データ生成）。
ユーザーの責務はタスクディレクトリを用意して `/generation-cycle <task_dir>` を呼ぶことだけ。`input/` が未整備なら原資料から作成し、整備済みならそのまま後続ステップへ進む。
...
`input/` が不足している場合は、まず `source/` に原資料があるか確認する。互換的に `raw/`, `docs/`, `input_sources/` も確認してよい。原資料があれば `input-prepare` から開始する。原資料もない場合は、不足ファイルを具体的に報告して停止する。
...
`description` は `"<step_name> 実行"` のように短く（例: `"input-prepare 実行"`）。

PM は **複数のサブエージェントを並列起動しない**（依存関係があるため、必ず逐次）。

## 改善ループ（PM が判定・制御する）

改善ループの責務は **PM 側にある**。step4（`evaluate-refine`）は「評価して `quality_gate.json` と推奨修正を出す」役割であり、step4 が内部で延々と自己修正ループを回すことはしない。最終的な再生成の要否判断は PM が `quality_gate.json` で行う。

手順:

1. `evaluate-refine` 完了後、PM は `$ARGUMENTS/output/quality_gate.json` を **read** する。
2. `requires_refinement == true` の場合のみ、`generate` のサブエージェントを再起動して再生成し、続けて `evaluate-refine` を再実行する（`blocking_issues` と `summary` を原因として伝える）。
3. `requires_refinement == false`（または `status == "pass"`）なら、ループを終了して完了報告へ進む。

**打ち切り条件（一箇所で定義）**:

- 再生成（`generate` → `evaluate-refine` の再実行）は **最大 2 回まで**。
...
- **構造的な作り直し**（生成方式そのものの見直し等）が必要な場合は、PM が `quality_gate.json` の `blocking_issues` を根拠に `generate` を再委譲する。
...
```
[input-prepare] sub-agent 起動
[input-prepare] 完了: input/table_definition.csv, input/sample_data.csv, input/data_spec.md, input/constraints.md
[spec-ingest] sub-agent 起動
[spec-ingest] 完了: work/inferred_schema.json, work/constraint_plan.md
[generation-design] sub-agent 起動
...
```
...
- **evaluate-refine で `requires_refinement == true`**: 「改善ループ」節の手順・打ち切り条件（再生成は最大 2 回まで）に従って `generate` を再委譲する。上限到達後も解消しない場合は、残課題を `evaluation_report.md` に明記させてループを停止する。
