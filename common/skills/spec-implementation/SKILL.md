...
> パス・検証コマンド・参照ドキュメント・サブエージェントは規約
> `.claude/docs/dev-workflow-conventions.md` の役割に従う（既定値はそのまま例示）。以下の
> `generator/` `site/` `analyses.py` 等はこのリポジトリ固有の例で、別プロジェクトでは
> 該当するソース・生成物・検証コマンドに読み替える。code-reviewer 等が無ければ通常の
> レビューで代替してよい。

**仕様書**（規約 §3。既定 `docs/SPEC.md`）を唯一の正とし、1 回につき 1 つの作業単位だけを進める。
