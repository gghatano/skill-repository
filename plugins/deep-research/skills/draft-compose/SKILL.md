---
name: draft-compose
description: 収集済みの情報源と根拠だけを使って調査レポートの初稿を書く。source-collect の後に実行する。新たな検索は行わず、Source Note にある内容のみで構成し、主張が追跡できる形で出典を付ける。根拠のない数値や、不明点を埋める自然な文章を書かない。 例:「初稿を書いて」「集めた根拠でレポートにして」
version: 0.1
---
