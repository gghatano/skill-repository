---
name: multi-review
description: 初稿を4つの観点（根拠・網羅・反証・依頼適合）で独立にレビューし、指摘を Finding として構造化する。draft-compose の後に実行する。一つの曖昧なレビューにまとめず、失敗モードごとに Reviewer を分けて、指摘対象を具体的に特定する。 例:「初稿をレビューして」「観点別に見て」
version: 0.1
---
...
サブエージェントに委譲するときは、規約 `.claude/docs/subagent-monitoring.md` に従う。要点:

- **背景起動する。** ブロッキングで待つと、待っている間の状態が見えない。
- **一定間隔で状態を見る。** タスクの状態・成果物の更新時刻・`work/progress.json` の 3 点。
- **委譲プロンプトにハートビートを課す。** 5 分に 1 回 `work/progress.json` を更新させる。
- **停滞（15 分更新なし）を検知したら**、途中成果物を `work/stalled-<step>-<時刻>/` へ退避してから
  停止し、**同じステップを 1 回だけ**再起動する。2 回目の停滞では再起動せず、経過時間・
  最後の `phase`・退避先を報告して止まる。
