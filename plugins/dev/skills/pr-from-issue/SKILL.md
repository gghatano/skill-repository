...
> ブランチ・検証コマンド・参照ドキュメントは規約 `.claude/docs/dev-workflow-conventions.md`
> の役割に従う（既定値はそのまま例示）。`gh` コマンド・`develop` ベース・`Closes #n` 等は
> プロジェクトの運用に読み替える。

pr-prep SKILL の検証を通したうえで、Issue と PR を結びつける。

## 前提
- pr-prep の**全検証**が緑（規約 §1。既定例: pytest / ruff / ruff format --check / mypy / build）
