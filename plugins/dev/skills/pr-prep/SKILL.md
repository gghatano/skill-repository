...
> 検証コマンド・パス・参照ドキュメントは規約 `.claude/docs/dev-workflow-conventions.md`
> の役割に従う（既定値はそのまま例示）。別プロジェクトでは §1 検証コマンド等を読み替える。

1. ブランチを確認する（既定ブランチでないこと）
2. **全検証**を通しで実行し、出力を控える（規約 §1。既定例）:
   - `uv run pytest -q`（test）
   - `uv run ruff check .`（lint）
   - `uv run ruff format --check .`（format）
   - `uv run mypy <package>`（typecheck）
   - `make build` 等（build / 再現性スモーク）
